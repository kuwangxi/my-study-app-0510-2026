...
def add_wish_callback():
    ...
    time_val = st.session_state.get("t_type_wish_add") # セレクトボックスの値
...
def add_ng_callback():
    memo = st.session_state.get("ng_memo_in")
    date = st.session_state.get("ng_in")
    # time_selector_uiの内部キーを取得するのは難しいため、ここは保存のみリセット
    get_ng_ref().add({
        "roomKey": st.session_state.room_key,
...
with tab1:
    with st.expander("＋ 新しい場所を追加"):
        ...
        st.text_area("メモ (詳細・感想など)", key="input_memo_wish") # メモ欄追加
        time_selector_ui("wish_add")
        # on_clickを使用してコールバックを呼び出す（エラー回避）
    ...
    for item in sorted(wish_items, key=get_latest_activity_time, reverse=True):
        with st.container(border=True):
            ...
            if item.get("memo"): # メモを表示
                st.markdown(f'<div class="memo-text">📝 {item["memo"]}</div>', unsafe_allow_html=True)
            ...
            with c_msg: 
                with st.expander("💬 コメント・確定"):
                    ...
                    c_col2.button("送信", key=f"ncb_{item['id']}", use_container_width=True, on_click=add_comment_callback, args=(item['id'], user_name))
                    st.divider()
                    st.write("📅 予定を確定する")
...
with tab3:
    ...
    with st.expander("💰 共有貯金・家計簿"):
        # (ロジック部分は維持)
        total_expense = sum([f['amount'] for f in finances]); st.markdown(f"<h3 style='color: #10b981;'>現在の残高: ¥{total_expense:,} (仮表示)</h3>", unsafe_allow_html=True)
        ...
        with fc1:
            ...
            st.button("記録する", use_container_width=True, on_click=add_expense_callback)

# --- タブ4: NG日 ---
with tab4:
    st.date_input("日付", value=today_jst, key="ng_in")
    time_selector_ui("ng_time_in")
    st.text_input("メモ (任意)", key="ng_memo_in")
    ...
    for n in sorted(ng_dates, key=lambda x: x["date"]):
        with st.expander(f"🚫 {n['date']} - {n.get('userName', '不明')}"):
            if st.button("🗑️ 削除", key=f"del_ng_{n['id']}", use_container_width=True):
