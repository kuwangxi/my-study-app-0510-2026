  ...
  return (
    <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 relative mb-4 text-left">
      ...
      <div className="mt-5 border-t border-gray-100 pt-4">
        <p className="text-[10px] font-bold text-gray-400 mb-2 uppercase tracking-widest">Your Mood</p>
          ...
          {!isSelectingDate ? (
            ...
            <div className="w-full p-4 bg-slate-100 rounded-2xl border border-slate-200">
              <div className="flex justify-between items-center mb-3">
                <span className="text-xs font-bold text-slate-700">日程設定</span>
...
export default function App() {
  ...
  // 1. Initial Authentication Logic (RULE 3)
  useEffect(() => {
    ...
    const unsubscribeAuth = onAuthStateChanged(auth, (u) => {
      ...
      if (u) {
        ...
        const savedKey = localStorage.getItem('pair_note_key');

        if (keyInUrl) {
          setRoomKey(keyInUrl);
          localStorage.setItem('pair_note_key', keyInUrl);
          setIsLogged(true);
  ...
  // 2. Data Sync with Firestore (Guarded by user)
  useEffect(() => {
    ...
    const unsubNg = onSnapshot(ngRef, (snap) => {
      ...
      setNgDates(filtered);
    }, (err) => console.error("NG snapshot error:", err));
  ...
  const handleCreateRoom = async () => {
    ...
    try {
      ...
      setRoomKey(newKey);
      localStorage.setItem('pair_note_key', newKey);
      setIsLogged(true);
    } catch (err) {
  ...
  const handleJoinRoom = () => {
    ...
    if (trimmedKey.startsWith('LOV-')) {
      setRoomKey(trimmedKey);
      localStorage.setItem('pair_note_key', trimmedKey);
      setIsLogged(true);
    }
  };
      ...
      <div className="min-h-screen bg-slate-900 flex items-center justify-center p-6 text-white text-center">
        <div className="w-full max-w-sm space-y-8">
          <div className="space-y-4">
            ...
            <h1 className="text-2xl font-black">Secure Pair Note</h1>
            <p className="text-slate-400 text-sm">秘密鍵による共有空間</p>
          </div>
          ...
          <div className="bg-slate-800 p-6 rounded-[2rem] border border-slate-700 shadow-xl space-y-6">
            ...
            <div className="space-y-3">
              <div className="relative">
                ...
                <input 
                  type="text" placeholder="LOV-XXXX-XXXX-XXXX" value={inputKey} onChange={e=>setInputKey(e.target.value.toUpperCase())}
                  className="w-full bg-slate-900 border border-slate-700 rounded-2xl py-4 pl-12 pr-4 text-sm font-mono focus:border-rose-500 outline-none transition-all"
                />
              </div>
              <button onClick={handleJoinRoom} disabled={!inputKey.startsWith('LOV-')} className="w-full bg-slate-700 hover:bg-slate-600 disabled:opacity-30 py-4 rounded-2xl font-black text-sm transition-all">
                秘密鍵で入る
              </button>
            </div>
          </div>
          {!user && <p className="text-rose-400 text-[10px] font-bold">接続を確立しています...</p>}
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-24 text-center">
      ...
      <footer className="fixed bottom-6 left-1/2 -translate-x-1/2 w-[calc(100%-3rem)] max-w-md bg-slate-900 text-slate-500 p-4 rounded-2xl flex justify-between items-center shadow-2xl border border-slate-800">
        <div className="flex items-center gap-2">
          <Lock size={12} className="text-rose-500"/>
          <span className="text-[10px] font-mono tracking-tighter uppercase">SECURE KEY: {roomKey}</span>
        </div>
        <button onClick={() => { localStorage.clear(); window.location.reload(); }} className="text-[10px] font-black uppercase tracking-widest hover:text-white flex items-center gap-1 transition-colors">
